...
const update = (user, callback) => {
    const queryString = `UPDATE jsusers SET nume=?, prenume=?, email=?, datanastere=?, telefon=? WHERE id=?`;
    db_1.db.query(queryString, [user.nume, user.prenume, user.email, user.datanastere, user.telefon, user.id], (err, result) => {
        if (err) {
            callback(err);
        }
        callback(null);
